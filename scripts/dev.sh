...
# When this script lives at scripts/dev.sh, the repo root is the
# parent of this directory. We expose both names — SCRIPT_DIR
# remains for back-compat (older copies used it), and REPO_ROOT
# is the authoritative anchor for paths that must live at the
# repo root (backend/, frontend/, .env, .run/).
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
# Treat SCRIPT_DIR as REPO_ROOT for the rest of the script so
# existing path math (BACKEND_DIR, FRONTEND_DIR, LOG_DIR, etc.)
